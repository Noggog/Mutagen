...
namespace Mutagen.Bethesda.Starfield
{
        ...
        IReverbParametersGetter
    {
        ...
        public Guid? AudioBus => Payload.AudioBusLocation.HasValue ? new Guid(HeaderTranslation.ExtractSubrecordMemory(_recordData, Payload.AudioBusLocation.Value, _package.MetaData.Constants).Slice(0, 16)) : default(Guid?);
        public ReverbParameters.Class ReverbClass => EnumBinaryTranslation<ReverbParameters.Class, MutagenFrame, MutagenWriter>.Instance.ParseRecord(Payload.ReverbClassLocation, _recordData, _package, 4);

        internal partial class ReverbParametersRecordDataPayload
        {
            public int? AudioBusLocation;
            ...
            TypedParseParams translationParams = default)
        {
            ...
            switch (type.TypeInt)
            {
                case RecordTypeInts.RABG:
                {
                    _payload.Fields.AudioBusLocation = (stream.Position - offset);
                    return (int)ReverbParameters_FieldIndex.AudioBus;
                }
                case RecordTypeInts.ANAM:
                {
