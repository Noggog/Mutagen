...
{
    public partial P2Float GetObjectBoundsMinCustom() => Payload.NAM0Location.HasValue
        ? new P2Float(
            _recordData.Slice(Payload.NAM0Location.Value.Min).Float() / 4096f,
            _recordData.Slice(Payload.NAM0Location.Value.Min + 4).Float() / 4096f)
        : default;

    public partial P2Float GetObjectBoundsMaxCustom() => Payload.NAM9Location.HasValue
        ? new P2Float(
            _recordData.Slice(Payload.NAM9Location.Value.Min).Float() / 4096f,
            _recordData.Slice(Payload.NAM9Location.Value.Min + 4).Float() / 4096f)
        : default;
