...
public class WriteBuilderTests
{
    ...
    /// <summary>
    /// Reproduces a Spriggit deserialize failure: a patch lists masters where one of them is
    /// not present on disk in the data folder, but its master style is supplied via
    ...
    /// (which itself has its own master) used to fail to parse with
    /// MissingModMappingException("Master flag lookup was not provided.") because the
    /// master flag lookup short-circuits to null when any listing is missing on disk.
    /// </summary>
    [Theory, MutagenAutoData]
        ...
        DirectoryPath existingDataDir)
    {
        ...
        // Mirrors BlueprintShips-Starfield.esm in the original report (which has Starfield.esm as a master).
        ...
        // Patch references both as direct masters (mirroring the BSPF patch).
        var mod = new StarfieldMod(modKey, StarfieldRelease.Starfield);
    ...
    /// error — not the cryptic "Master flag lookup was not provided." that the bug used to surface.
    /// </summary>
    [Theory, MutagenAutoData]
